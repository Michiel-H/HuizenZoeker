"""Collector for VB&T Verhuurmakelaars listings."""
...
class VBTCollector(BaseCollector):
    SOURCE_NAME = "VB&T"
    BASE_URL = "https://www.vbtmakelaars.nl"
    ...
    def collect(self) -> list[RawListing]:
        ...
        try:
            ...
            items = soup.select("[class*='property'], [class*='object'], [class*='woning'], article, .card")
            for item in items:
                try:
                    ...
                    logger.debug(f"[VBT] Parse error: {e}")
        except Exception as e:
            logger.error(f"[VBT] Fetch error: {e}")
        return listings

    def _parse_item(self, item) -> RawListing | None:
        title_el = item.select_one("a[href], h2, h3, [class*='title'], [class*='address']")
        if not title_el:
            return None
        ...
        location = ""
        loc_el = item.select_one("[class*='location'], [class*='address'], [class*='plaats']")
        if loc_el:
            location = loc_el.get_text(strip=True)

        area_m2 = None
        for el in item.select("[class*='feature'], [class*='kenmerk'], span, li"):
