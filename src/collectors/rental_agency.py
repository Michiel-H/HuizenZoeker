"""Collector for The Rental Agency Amsterdam listings."""
...
class RentalAgencyCollector(BaseCollector):
    SOURCE_NAME = "The Rental Agency"
    BASE_URL = "https://www.therentalagency.nl"

    def collect(self) -> list[RawListing]:
        listings: list[RawListing] = []
        url = f"{self.BASE_URL}/en/residences/for-rent/amsterdam"
        try:
            html = self.fetch_page(url)
            soup = BeautifulSoup(html, "lxml")
            items = soup.select("[class*='residence'], [class*='property'], article")
            for item in items:
                try:
                    ...
                    logger.debug(f"[RentalAgency] Parse error: {e}")
        except Exception as e:
            logger.error(f"[RentalAgency] Fetch error: {e}")
        return listings
