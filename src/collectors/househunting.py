"""Collector for HouseHunting Amsterdam listings."""
...
class HouseHuntingCollector(BaseCollector):
    SOURCE_NAME = "HouseHunting"
    BASE_URL = "https://www.househunting.nl"

    def collect(self) -> list[RawListing]:
        listings: list[RawListing] = []
        url = f"{self.BASE_URL}/woningaanbod/amsterdam/huur/"
        try:
            html = self.fetch_page(url)
            soup = BeautifulSoup(html, "lxml")
            items = soup.select("[class*='property'], [class*='object'], [class*='residence'], article")
            for item in items:
                try:
                    ...
                    logger.debug(f"[HouseHunting] Parse error: {e}")
        except Exception as e:
            logger.error(f"[HouseHunting] Fetch error: {e}")
        return listings
