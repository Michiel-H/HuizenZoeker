"""Collector for Hausing.com rental listings."""
...
class HausingCollector(BaseCollector):
    SOURCE_NAME = "Hausing"
    BASE_URL = "https://www.hausing.com"

    def collect(self) -> list[RawListing]:
        listings: list[RawListing] = []
        url = f"{self.BASE_URL}/en/rental-listings/amsterdam"
        try:
            html = self.fetch_page(url)
            soup = BeautifulSoup(html, "lxml")
            items = soup.select("[class*='property-card'], [class*='listing-card'], [class*='house-card']")
            if not items:
            ...
            for item in items:
                try:
                    ...
                    logger.debug(f"[Hausing] Parse error: {e}")
        except Exception as e:
            logger.error(f"[Hausing] Fetch error: {e}")
        return listings
