"""Collector for Amsterdam Rentals listings."""
...
class AmsterdamRentalsCollector(BaseCollector):
    SOURCE_NAME = "Amsterdam Rentals"
    BASE_URL = "https://www.amsterdamrentals.com"
    ...
    def collect(self) -> list[RawListing]:
        ...
        try:
            ...
            items = soup.select("[class*='property'], [class*='listing'], article, .card")
            for item in items:
                try:
                    ...
                    logger.debug(f"[AmsterdamRentals] Parse error: {e}")
        except Exception as e:
            logger.error(f"[AmsterdamRentals] Fetch error: {e}")
        return listings
    ...
    def _parse_item(self, item) -> RawListing | None:
        ...
        area_m2 = None
        for el in item.select("[class*='feature'], [class*='detail'], span"):
