"""Collector for PUUR Makelaars listings."""
...
class PuurMakelaarsCollector(BaseCollector):
    SOURCE_NAME = "PUUR Makelaars"
    BASE_URL = "https://www.puurmakelaars.nl"
    ...
    def collect(self) -> list[RawListing]:
        ...
        try:
            ...
            items = soup.select("[class*='property'], [class*='object'], article, .card, [class*='woning']")
            for item in items:
                try:
                    ...
                    logger.debug(f"[PUUR] Parse error: {e}")
        except Exception as e:
            logger.error(f"[PUUR] Fetch error: {e}")
        return listings
