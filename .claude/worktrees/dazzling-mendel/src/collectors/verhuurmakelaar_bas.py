"""Collector for Verhuurmakelaar Bas listings."""
...
class VerhuurmakelaarBasCollector(BaseCollector):
    SOURCE_NAME = "Verhuurmakelaar Bas"
    BASE_URL = "https://www.verhuurbas.nl"

    def collect(self) -> list[RawListing]:
        listings: list[RawListing] = []
        url = f"{self.BASE_URL}/woningaanbod/huur/amsterdam"
        try:
            html = self.fetch_page(url)
            soup = BeautifulSoup(html, "lxml")
            items = soup.select("[class*='property'], [class*='object'], [class*='woning'], article")
            for item in items:
                try:
                    ...
                    logger.debug(f"[VerhuurBas] Parse error: {e}")
        except Exception as e:
            logger.error(f"[VerhuurBas] Fetch error: {e}")
        return listings
