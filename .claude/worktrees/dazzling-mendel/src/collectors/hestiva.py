"""Collector for Hestiva (particulier platform) listings."""
...
class HestivaCollector(BaseCollector):
    SOURCE_NAME = "Hestiva"
    BASE_URL = "https://www.hestiva.nl"
    ...
    def collect(self) -> list[RawListing]:
        ...
        try:
            ...
            items = soup.select("[class*='property'], [class*='listing'], [class*='result'], article, .card")
            for item in items:
                try:
                    ...
                    logger.debug(f"[Hestiva] Parse error: {e}")
        except Exception as e:
            logger.error(f"[Hestiva] Fetch error: {e}")
        return listings
