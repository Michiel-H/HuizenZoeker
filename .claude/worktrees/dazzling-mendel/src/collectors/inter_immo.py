"""Collector for Inter Immo Real Estate Agents listings."""
...
class InterImmoCollector(BaseCollector):
    SOURCE_NAME = "Inter Immo"
    BASE_URL = "https://www.interimmo.nl"

    def collect(self) -> list[RawListing]:
        listings: list[RawListing] = []
        url = f"{self.BASE_URL}/aanbod/huur/amsterdam/"
        try:
            ...
            for item in items:
                try:
                    ...
                    logger.debug(f"[InterImmo] Parse error: {e}")
        except Exception as e:
            logger.error(f"[InterImmo] Fetch error: {e}")
        return listings
